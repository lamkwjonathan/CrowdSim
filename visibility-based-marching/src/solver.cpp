...
void Solver::visibilityBasedSolver() {
  ...
  //vbm accepts input coordinates with (0,0) at the bottom left, processes calculations internally with (0,0) at the top left, and returns output files with (0,0) at the bottom left
  vectorFrom_.reset(nx_ * 2, ny_, 0);
  for (int j = 0; j < ny_; ++j)
  {
      for (int i = 0; i < nx_; ++i)
      {
          vectorFrom_(i*2, j) = lightSources_[cameFrom_(i, j)].x;
          //vectorFrom_(i*2 + 1, j) = lightSources_[cameFrom_(i, j)].y; 
          vectorFrom_(i*2 + 1, j) = ny_ - 1 - lightSources_[cameFrom_(i, j)].y;
      }
  }
...
void Solver::saveResults(const std::vector<point> &resultingPath,
  ...
  if (methodName == "visibilityBased") {
    ...
    if (sharedConfig_->saveVectorFrom) {
        outputFilePath = "./output/" + methodName + "_vectorFrom.txt";
        std::fstream of4(outputFilePath, std::ios::out | std::ios::trunc);
        if (!of4.is_open()) {
            std::cerr << "Failed to open output file " << outputFilePath
                << std::endl;
            return;
        ...
        of4.close();
        if (!sharedConfig_->silent) {
            std::cout << "Saved " + methodName + " vectorFrom_" << std::endl;
        }
    }
    return;
  }
